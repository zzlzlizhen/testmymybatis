...
<script type="text/javascript">
		...
		table = $('#dataTables').dataTable(
						{
							...
							"sAjaxSource" : "${appctx}/excepLogController/getExcepLogs",
							...
							"fnServerParams": function (aoData) {  //查询条件
			                    aoData.push(
			                    	...
			                    	{ "name": "exceptionDesc", "value": $("#exceptionDesc").val()},
	...
	function viewFun(id){
		$("#myModal .modal-header").find("h3").html("异常日志详情");
		$("#myModal .modal-footer #saveBtn").html("保存");
		$("#myModal .modal-footer #closeBtn").html("取消");
		$("#saveBtn").hide();
		$("#myModal").modal({
		    remote: "${appctx}/excepLogController/viewPage/"+id
		});
	}
	...
	function batchDelSubmit(){
		$.ajax({
	        url: '${appctx}/excepLogController/batchDel',
	        async: true,
	        contentType:"application/json",
	        type: 'POST',
	        data: JSON.stringify({exceptionDesc:batchDelId}),
			...
			<div class="box col-md-12">
				<div class="box-inner">
					<div class="box-header well" data-original-title="">
						<h2>
							<i class="glyphicon glyphicon-user"></i>异常日志
						</h2>
					</div>
						...
						<div class="btn-toolbar">
							<div class="pull-right">
								<div class="input-append">
									用户名：<input type="text" placeholder="用户名" id="name" /> 异常描述：<input
										type="text" placeholder="异常描述" id="exceptionDesc" /> 时间：<input
										type="text" placeholder="开始时间" id="startTime"
										onClick="WdatePicker({maxDate:'#F{$dp.$D(\'endTime\')||\'%y-%M-%d\'}',dateFmt:'yyyy-MM-dd'})" />
									-- <input type="text" placeholder="结束时间" id="endTime"
										onClick="WdatePicker({minDate:'#F{$dp.$D(\'startTime\')}',maxDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd'})" />
									<a class="btn btn-primary" href="#" onclick="searchFun()">
										<i class="glyphicon glyphicon-search"></i>查询
									</a>
								</div>
							</div>
							<a class="btn btn-danger" href="#" onclick="batchDelFun()"
