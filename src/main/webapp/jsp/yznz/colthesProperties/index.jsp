...
<script type="text/javascript">
		...
		table = $('#dataTables').dataTable(
						{
							...
							"sAjaxSource" : "${appctx}/yznz/colthesProperties/getParams",
	...
	function transProperties(properties){
		if(properties.indexOf("YZNZ_CLOTHESPROPERTIES_SIZE")>-1){
			return "尺寸";
		}
		else if(properties.indexOf("YZNZ_CLOTHESPROPERTIES_COLOUR")>-1){
			return "颜色";
		}
		else if(properties.indexOf("YZNZ_CLOTHESPROPERTIES_STYLE")>-1){
			return "风格";
		}
		else if(properties.indexOf("YZNZ_CLOTHESPROPERTIES_CATEGORY")>-1){
			return "类型";
		}else{
			return "未知";
		}
	...
	function viewFun(id){
		...
		$("#saveBtn").hide();
		$("#myModal").modal({
		    remote: "${appctx}/yznz/colthesProperties/viewPage/"+id
		});
	}
	function addFun(){
		...
		$("#saveBtn").show();
		$("#myModal").modal({
		    remote: "${appctx}/yznz/colthesProperties/addPage"
		});
	}
	function editFun(id){
		...
		$("#saveBtn").show();
		$("#myModal").modal({
		    remote: "${appctx}/yznz/colthesProperties/editPage/"+id
		});
	}
	function delFun(id){
		delId = id;
	...
	function delSubmit(){
		$.ajax({
	        url: '${appctx}/yznz/colthesProperties/del',
	        async: true,
	...
	function batchDelSubmit(){
		$.ajax({
	        url: '${appctx}/yznz/colthesProperties/batchDel',
	        async: true,
	        contentType:"application/json",
	        type: 'POST',
						...
						<div class="btn-toolbar">
							<div class="pull-right">
								<div class="input-append">
									衣服属性：<select id="searchId">
									  	  	<option value="">全部</option>
									  	  	<option value="YZNZ_CLOTHESPROPERTIES_SIZE">尺寸</option>
									  	  	<option value="YZNZ_CLOTHESPROPERTIES_COLOUR">颜色</option>
									  	  	<option value="YZNZ_CLOTHESPROPERTIES_CATEGORY">类型</option>
									  	  	<option value="YZNZ_CLOTHESPROPERTIES_STYLE">风格</option>
										</select>
